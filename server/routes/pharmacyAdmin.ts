...
export function registerPharmacyAdminRoutes(app: Express) {
  ...
  app.post("/api/pharmacy/orders/:orderId/mark-paid", async (req: Request, res: Response) => {
    try {
      ...
      // Update payment status and history
      const orderResult = await db
        .update(orders)
  ...
  app.post("/api/pharmacy/orders/:orderId/status", async (req: Request, res: Response) => {
    try {
      ...
      const validStatuses = ["preparing", "ready", "out_for_delivery", "delivered", "awaiting_proof", "proof_submitted"];
    ...
    try {
      ...
      const stats = {
        ...
        totalRevenue: allOrders
          .filter(o => ["delivered", "ready", "out_for_delivery"].includes(o.status) || o.paymentStatus === "paid")
