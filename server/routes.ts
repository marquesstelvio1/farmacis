...
import { db } from "./db";
import { prescriptions, orders, systemSettings, adminUsers } from "@shared/schema";
...
async function seedDatabase() {
  ...
  if (existingAdmins.length === 0) {
    ...
    const ownerPassword = await bcrypt.hash("super_secure_password_123", 10);

    // Create super-super admin (System Owner)
    await db.insert(adminUsers).values({
      email: "stelvio@farmacis.com",
      password: ownerPassword,
      name: "Stelvio (Proprietário)",
      role: "super_admin",
      isSystemOwner: true
    });
    ...
    console.log("✓ Admin users created: stelvio@farmacis.com and admin@farmacis.com");
  }
