...
const getCurrentPrice = () => {
  ...
  const getOriginLabel = (origin: string) => {
    switch (origin) {
      ...
      default: return { label: "Padrão", flag: "🏥", color: "bg-green-100 text-green-700 border-green-300" };
    }
  };
  ...
  const originInfo = getOriginLabel(selectedOrigin);
    ...
    <ArrowLeft className="w-5 h-5" />
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        <div className="grid md:grid-cols-2 gap-8">
          ...
          >
            ...
            <div>
              ...
              {product.dosage && (
                <p className="text-lg text-green-600 font-semibold">{product.dosage}</p>
              )}
            </div>
              ...
              {hasMultipleOrigins && (
                <div className="mb-4">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    <button
                      onClick={() => setSelectedOrigin("default")}
              ...
              <Card className="p-4 border-slate-200">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <h3 className="font-bold text-slate-800">{product.pharmacyName}</h3>
            ...
            <Button
              ...
              className="w-full h-14 bg-green-600 hover:bg-green-700 text-white text-lg font-bold rounded-xl shadow-lg shadow-green-500/30"
            >
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="flex items-center gap-2 text-sm text-slate-600">
                ...
                <span>Origem: {originInfo.label}</span>
              </div>
            </div>
          </motion.div>
        </div>
        ...
        <motion.div
          ...
          className="mt-12"
        >
          <h2 className="text-xl font-bold text-slate-800 mb-4">Descrição do Produto</h2>
          <Card className="p-6 border-slate-200">
            <p className="text-slate-600 leading-relaxed">{product.description}</p>

            {product.activeIngredient && (
              <div className="mt-6 pt-6 border-t border-slate-100">
                <h3 className="font-semibold text-slate-800 mb-2">Princípio Ativo</h3>
                <p className="text-slate-600">{product.activeIngredient}</p>
              </div>
            )}

            {product.diseases && product.diseases.length > 0 && (
              <div className="mt-6 pt-6 border-t border-slate-100">
                <h3 className="font-semibold text-slate-800 mb-3">Indicações Terapêuticas</h3>
                <div className="flex flex-wrap gap-2">
                  {product.diseases.map((disease) => (
                    <Badge key={disease} variant="secondary" className="px-3 py-1">
                      {disease}
                    </Badge>
                  ))}
                </div>
